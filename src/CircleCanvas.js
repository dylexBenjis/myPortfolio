...
import { BackgroundImageWrapper } from './App';
...
export const debounce = (callback, delay = 250) => {
  ...
  export const throttle = (callback, limit = 250) => {
    ...
    return () => {
      ...
      console.log(circleCenterCoordinates.x);
      ...
      const m = {
        ...
        createMachine: (ctx,  toggle) => {
          ...
          document.body.style.backgroundColor = m.toggle ? COLORS.midnightBlack : COLORS.white;
          ...
          if (circleCenterCoordinates.x == null || circleCenterCoordinates.y == null) {
            m.radiusMultiplier = toggle ? 0 : m.maxRadiusMultiplier;
          }
      
          return m.start;
        },
        start: () => (m.toggle ? m.shrinkCircle : m.growCircle),
        ...
        verifyCircleBounds: () => {
          if (
            (m.radiusMultiplier <= 0 && m.toggle) ||
            (m.radiusMultiplier >= m.maxRadiusMultiplier && !m.toggle)
          ) {
            // just fill the canvas - at the limit. no more drawing is necessary.
            m.ctx.fillStyle = m.toggle ? COLORS.midnightBlack : COLORS.white;
            m.ctx.fillRect(0, 0, m.width, m.height);
            // reset radius multiplier values in case we overshoot (can get negative value when shrinking)
            m.radiusMultiplier = m.toggle ? 0 : m.maxRadiusMultiplier;
...
const CircleCanvas = ({ toggle}) => {
    ...
    useEffect(() => {
      ...
      let stateMachine = m.createMachine(ctx,  toggle);
      ...
      const handleResize = () => {
        ...
        stateMachine = m.createMachine(ctx,  toggle);
        stateMachineRunner();
      };
    ...
    return <canvas className={BackgroundImageWrapper} ref={canvasRef} />;
  };
